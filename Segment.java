...
//Displayer 3 digits with 7 segments for each one;
//Used for the time and the flags
...
public class Segment extends JPanel {
  ...
  */
  private boolean[][] chiffres = {
    //Table of truth of all diferents numbers
  ...
  private int valeur;
  final private Color AFFICHE = new Color(255,0,0);//color of the segment when it's turned on
  final private Color CACHE = new Color(80,0,0);//color of the segment when it's turned off
  private Border border1;
  	...
  	//Set a specific value into the segment displayer
  	...
  	public void paintComponent(Graphics g) {
  		...
  		int[] segments = new int[3];//The digit displayed by all displayer
  		segments[0]=valeur/100; segments[1]=(valeur/10)%10; segments[2]=valeur%10;
  		for (int i=0; i<3; i++) {//for each segment
  			int n=segments[i];
